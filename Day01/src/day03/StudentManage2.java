...
//			case "추가" :  // case 2개 겹치기도 가능
			...
			case "4" :
				...
				if(index >=0 && index <= count-1) { // 출력이 가능한 조건 index => 0~count-1
					...
					System.out.println("현 위치에 회원정보가 없습니다");
				}
			...
			case "5" :
				...
				if(index >=0 && index <= count-1) {
					...
					System.out.println("현 위치에 수정할 정보 없음");
				}
				break;
			...
			case "6" :
				...
				if(index >= 0 && index <= count-1) {
					...
					System.out.println(nameList[index] + "님의 정보가 삭제되었습니다");
					
					for(int i = index; i<nameList.length-1; i++) { // count == length -> i < count-1;
